...
from qt_ui.stim_config import PulseParameters
...
class MyStaticMplCanvas(MyMplCanvas):
    ...
    def updateParams(self, parameters: PulseParameters):
        samplerate = 44100
        x_limit = .10
        ...
        while len(y) / samplerate < x_limit:
            theta = np.linspace(0, 2 * np.pi * parameters.pulse_width, int(samplerate / parameters.carrier_frequency * parameters.pulse_width)) + gen.uniform(0, np.pi * 2)
            pulse = np.cos(theta) * stim_math.pulse.create_pulse_envelope_half_circle(len(theta))
            y += list(pulse)
            pause_length = samplerate / parameters.pulse_frequency - len(pulse)
            pause_length *= gen.uniform(1 - parameters.pulse_interval_random, 1 + parameters.pulse_interval_random)
            if pause_length > 0:
...
class PulseSettingsWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        carrier_slider = QtWidgets.QDoubleSpinBox(minimum=stim_math.limits.PulseCarrierFrequency.min,
                                                  maximum=stim_math.limits.PulseCarrierFrequency.max)
        ...
        self.pulseSettingsChanged.connect(self.mpl_canvas.updateParams)
        self.settings_changed()

    pulseSettingsChanged = QtCore.pyqtSignal(PulseParameters)

    def settings_changed(self):
        params = PulseParameters(
            self.carrier.value(),
            self.pulse_freq_slider.value(),
            self.pulse_width_slider.value(),
            self.pulse_interval_random.value() / 100.0,
            0, #self.polarity.currentData(),
            0, #self.device_emulation_mode.currentData(),
            0, #self.pulse_phase_offset_increment.value(),
        )
        self.pulseSettingsChanged.emit(params)
