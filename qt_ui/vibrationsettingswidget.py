...
from qt_ui.stim_config import VibrationParameters
...
class MyStaticMplCanvas(MyMplCanvas):
    ...
    def updateParams(self, parameters: VibrationParameters):
        t = np.linspace(0, 1, 10000)

        def create_vibration_signal(timeline, frequency, modulation, l_r_bias, h_l_bias):
        ...
        if parameters.vibration_1_enabled:
            y *= create_vibration_signal(t, parameters.vibration_1_freq, parameters.vibration_1_strength, parameters.vibration_1_left_right_bias, parameters.vibration_1_high_low_bias)
        if parameters.vibration_2_enabled:
            y *= create_vibration_signal(t, parameters.vibration_2_freq, parameters.vibration_2_strength, parameters.vibration_2_left_right_bias, parameters.vibration_2_high_low_bias)
...
class VibrationSettingsWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.vibrationSettingsChanged.connect(self.mpl_canvas.updateParams)
        self.settings_changed()

    vibrationSettingsChanged = QtCore.pyqtSignal(VibrationParameters)

    def settings_changed(self):
        params = VibrationParameters(
            self.gb1.isChecked(),
        ...
        self.vibrationSettingsChanged.emit(params)
