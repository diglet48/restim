"""
Pattern Control Service
Handles pattern application and validation logic.
Separated from UI to enable reuse and testing.
"""
import logging
...
from qt_ui.patterns.threephase.base import get_registered_patterns, get_patterns_by_category, get_all_categories
import qt_ui.settings

logger = logging.getLogger('restim.pattern_service')
...
class PatternControlService:
    def __init__(self, max_history_size: int = 100):
        self.activity_history: List[ActivityRecord] = []
        self.max_history_size = max_history_size
        self._pattern_registry = get_registered_patterns()
    
    def get_available_patterns(self, respect_user_preferences: bool = False) -> List[Dict[str, Any]]:
        patterns = []
        priority_patterns = []
        other_patterns = []
        
        # Get enabled patterns from settings
        enabled_patterns = qt_ui.settings.pattern_enabled.get()
        
        for pattern_name, pattern_class in self._pattern_registry.items():
            pattern_info = {
                "name": pattern_class.display_name if hasattr(pattern_class, 'display_name') else pattern_name,
            ...
            # Check if pattern should be filtered out based on user preferences
            if respect_user_preferences:
                # Mouse and Circle patterns are always enabled
                if pattern_class.__name__ not in ['MousePattern', 'CirclePattern']:
                    # For other patterns, check if they're disabled in settings
                    # Default to enabled if not in settings
                    pattern_enabled = enabled_patterns.get(pattern_info["name"], True)
                    if not pattern_enabled:
                        continue  # Skip disabled patterns
            
            # Put Mouse and Circle first, then sort the rest alphabetically
            if pattern_class.__name__ in ['MousePattern', 'CirclePattern']:
                priority_patterns.append(pattern_info)
            else:
                other_patterns.append(pattern_info)
        
        # Sort priority patterns (Mouse first, then Circle)
        priority_patterns.sort(key=lambda x: (x['class_name'] != 'MousePattern', x['name']))
        
        # Sort other patterns alphabetically by display name
        other_patterns.sort(key=lambda x: x['name'])
        
        # Combine: priority patterns first, then others
        patterns = priority_patterns + other_patterns
        
        return patterns
    
    def get_patterns_by_category(self, category: str) -> List[Dict[str, Any]]:
        """Get all patterns in a specific category"""
        category_patterns = get_patterns_by_category(category)
        patterns = []
        for pattern_name, pattern_class in category_patterns.items():
            patterns.append({
                "name": pattern_class.display_name if hasattr(pattern_class, 'display_name') else pattern_name,
                "description": getattr(pattern_class, 'description', pattern_name),
                "category": category,
    ...
    def get_available_categories(self) -> List[str]:
        ...
        return sorted(list(get_all_categories()))
    ...
    def is_pattern_enabled(self, pattern_name: str) -> bool:
        """Check if a pattern is enabled in user preferences"""
        enabled_patterns = qt_ui.settings.pattern_enabled.get()
        return enabled_patterns.get(pattern_name, True)  # Default to enabled
    
    def set_pattern_enabled(self, pattern_name: str, enabled: bool):
        """Set pattern enabled state in user preferences"""
        enabled_patterns = qt_ui.settings.pattern_enabled.get()
        # Create a new dictionary to avoid reference issues
        new_patterns = enabled_patterns.copy()
        new_patterns[pattern_name] = enabled
        qt_ui.settings.pattern_enabled.set(new_patterns)
        logger.info(f"Pattern '{pattern_name}' {'enabled' if enabled else 'disabled'}")
    
    def reset_pattern_preferences(self):
        """Reset all patterns to enabled state"""
        qt_ui.settings.pattern_enabled.set({})
        logger.info("Pattern preferences reset to defaults (all enabled)")
