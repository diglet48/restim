...
def generate_audio(timeline, alpha, beta, frequency,
    ...
    L, R = threephase.ContinuousSineWaveform.generate(timeline, frequency, alpha, beta)
    ...
    if modulation_1 is not None:
        L, R = modulation_1.modulate(timeline, L, R)
    if modulation_2 is not None:
        L, R = modulation_2.modulate(timeline, L, R)
